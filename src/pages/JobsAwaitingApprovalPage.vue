...
const search = ref('')
const userNames = ref({})
const regular = useApiList('/api/jobs', { per_page: 50, status: 'pending_review' })
const freelance = useApiList('/api/freelance-jobs', { per_page: 50, status: 'pending_review' })
...
const filteredJobs = computed(() => {
  ...
  return jobs.value.filter((job) => !q || [job.status, job.applicationEmail, job.location, job.title, job.companyName].filter(Boolean).join(' ').toLowerCase().includes(q))
})
