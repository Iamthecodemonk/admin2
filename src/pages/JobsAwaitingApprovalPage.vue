...
import { jobCompanyName, jobDetailsRows, jobEmail, jobTitle, loadJobsByBucket, updateAdminJobStatus } from '../services/jobs'
...
const filteredJobs = computed(() => {
  ...
  return jobs.value.filter((job) => !q || [job.status, jobEmail(job), job.location, job.title, jobCompanyName(job)].filter(Boolean).join(' ').toLowerCase().includes(q))
})
...
async function loadCurrentJobs() {
  ...
  error.value = ''
  try {
    const loadedJobs = await loadJobsByBucket(type.value, 'pending')
    ...
    <div class="container">
      <div class="filters pb-3">
        <div class="d-flex flex-wrap align-items-center justify-content-between pb-3">
          <h3 class="fs-22 fw-medium">Manage Jobs Awaiting Approval</h3>
        </div>
        <div class="d-flex flex-wrap align-items-center justify-content-between">
          <form method="post" class="me-3 w-25" @submit.prevent>
            <div class="form-group">
              <input v-model="search" class="form-control form--control form-control-sm h-auto lh-34" type="text" name="search" placeholder="status, email, location, username, company">
              <button class="form-btn" type="button"><i class="la la-search"></i></button>
            </div>
          </form>
          <div class="btn-group btn--group mb-3">
            <input id="pendingRegular" v-model="type" value="regular" type="radio" class="btn-check" name="jobType">
            <label class="btn btn-outline-primary" for="pendingRegular">Regular Jobs</label>
            <input id="pendingFreelance" v-model="type" value="freelance" type="radio" class="btn-check" name="jobType">
            <label class="btn btn-outline-primary" for="pendingFreelance">Freelance Jobs</label>
          </div>
        </div>
      </div>
      ...
      <div class="table-responsive">
        <table class="table table-bordered">
          <thead>
            <tr>
              <th>#</th>
              <th>post user (profile link)</th>
              <th>Company(view job details)</th>
              <th>Location</th>
              <th>job email</th>
              <th>Status - <br>pending/approved/deleted/suspended</th>
              <th>Action</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(job, index) in filteredJobs" :key="job.id">
              ...
              <td><a href="" data-bs-toggle="modal" data-bs-target="#jobDetailsModal" @click.prevent="showJobDetails(job)">{{ jobCompanyName(job) || jobTitle(job) }}</a></td>
              <td>{{ job.location }}</td>
              <td>{{ jobEmail(job) }}</td>
              <td>{{ job.status }}</td>
              <td>
            ...
            <tr v-if="!loading && filteredJobs.length === 0"><td colspan="7">No jobs found.</td></tr>
          </tbody>
        </table>
      </div>
    </div>
